...
use ratatui::{
	...
	symbols::{
		block::FULL,
		line::{DOUBLE_HORIZONTAL, THICK_VERTICAL, VERTICAL}
	},
	widgets::Widget,
	Frame
};
...
impl Scrollbar {
	...
	fn render_vertical(self, area: Rect, buf: &mut Buffer) {
		...
		for y in bar_top..(bar_top + bar_height) {
			buf.set_string(right, y, VERTICAL, self.style_bar);
		}
		...
		buf.set_string(right, bar_top + pos, THICK_VERTICAL, self.style_pos);
	}
